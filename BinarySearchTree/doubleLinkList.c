...
{
    #if 0
    return ddoubleLinkListGetAppointPosVal(pList, 0, pVal);
    #else

    #endif
}
/* 获取链表 尾位置值 */
int doubleLinkListGettTailVal(doubleLinkList * pList, ELEMENTTYPE *pVal)
{
    //return doubleLinkListGetAppointPosVal(pList, pList->len, *pList);
}
/* 获取链表 指定位置值 */
